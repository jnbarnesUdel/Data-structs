...
class hashMap {
	...
	int collHash1(hashNode *n, int count); // getting index with collision method 1 (note – you may modify the
	//parameters if you don’t need some/need more)
	int collHash2(hashNode *n, int count); // getting index with collision method 2 (note – you may modify the
	//parameters if you don’t need some/need more)
